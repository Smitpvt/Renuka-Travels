  ...
  return (
    <section id="fleet-preview" className="py-20 bg-slate-50 scroll-mt-24 px-6 border-y border-slate-100">
      <div className="max-w-[1280px] mx-auto">
        ...
        {/* Categories Grid */}
        <div className="flex md:grid md:grid-cols-3 gap-6 md:gap-8 overflow-x-auto md:overflow-x-visible snap-x snap-mandatory scrollbar-none pb-4 -mx-6 px-6 md:mx-0 md:px-0">
          {fleetCategories.map((cat, i) => (
            <div key={i} className="w-[82vw] sm:w-[350px] md:w-auto flex-shrink-0 snap-start">
              <motion.div
                whileHover={{ y: -5 }}
                transition={{ duration: 0.3 }}
                className="bg-white rounded-3xl overflow-hidden border border-slate-200/55 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between h-full"
              >
                <div>
                  {/* Image */}
